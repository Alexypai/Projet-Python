...
def newUser(new_log_user, new_password_user):
    ...
    c.execute(
        ...
        (new_log_user, new_password_user))
    connect.commit()
    connect.close()
...
# Application index with setting the id_project variable to zero when a user
# has finished creating a team
@app.route('/Index')
def index():
    id_Project = None
    print(id_Project)
...
# Function allowing the addition of the named project in the team creation page
# and assigning an id to this new project
def insert_team(new_team):
    ...
    c2 = connect.cursor()
    c2.execute(
        "SELECT PROJECTS_ID FROM PROJECTS WHERE PROJECTS_name=?",
        (new_team,))
    x = c2.fetchone()
    global id_Project
    id_Project = x[0]
...
# Whenever an employee has been added to a team the page will regenerate. To
# leave this page, click on the end creation of a team that returns to the
# application index.
...
# Function allowing the addition of an employee with the most available time in
# the team previously created, according to the role chosit by the user. When
# added, this function inserts in the USERS_BY_PROJECT table: the employee id
# with the most time available, the project id previously created and the time
...
def insert_user(id_Project, time_user, role_user):
    ...
    c.execute(
        ...
        "USERS INNER JOIN ("
        "SELECT USERS_BY_PROJECT.USERS_ID, Sum(USERS_BY_PROJECT.TIME) AS SBU "
        ...
        "ON RESPONSIBILITIES.Responsibilities_ID = USERS.Responsibilities_ID) "
    ...
    c2.execute(
        ...
        "VALUES (?,?,?)", (time_user, user_select, id_Project))
    connect.commit()
    connect.close()
...
def msgA():
    msgV = (
        "Employé Ajouté ! Vous pouvez continuer ou valide votre equipe "
        "en cliquant sur fin création d'equipe")
    return msgV


def msgD():
    msgV = (
        "Equipe supprimé ! Vous pouvez continuer ou utilisé le menu ci dessus "
