...
def Employe():
    ...
    return render_template('annuaire.html',donneesEmploye=donneesEmploye)
...
def insert_user(id_Project,time_user, role_user):
    ...
    user_select = x2[0]
    print(user_select)
    #If the user needs human resources people for example, thanks to this condition, the function does not add the same person twice to the same project
    if not norepeat:
